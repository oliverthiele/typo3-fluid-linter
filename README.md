...
- **CDATA section detection** — flags `<![CDATA[...]]>` inside `<f:comment>` blocks, the old pattern for safely commenting out Fluid code — CDATA told the parser not to interpret the block at all. CDATA is not removed in Fluid 5; what ended is Fluid *stripping* it, so the construct no longer comments anything out and logs a deprecation on every render from TYPO3 13.4.21 on — potentially very many entries in a template set that used the idiom consistently. The replacement is a plain `<f:comment>`, which ignores Fluid syntax errors by itself since TYPO3 13.3; legitimate CDATA in XML/RSS templates and `{{{expression}}}` output syntax are not flagged; fixable with `--fix`
...
| `<![CDATA[` inside `<f:comment>` — no longer stripped in Fluid 5, logs a deprecation from TYPO3 13.4.21 | `cdata-section` | error | all |
